...
rankhospital <- function(state, outcome, num = "best", outcome.hospital,
                         states) {
    col.id <- 11
...
rankall <- function(outcome, num = "best") {
    ...
    for (state in states) {
        hospital.name <- rankhospital(state, outcome, num, outcome.hospital,
                                      states)
        df <- rbind(df, data.frame(hospital = hospital.name, state = state))
    }
